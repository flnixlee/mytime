...
public class InfoMapEncrypt
{
    ...
    public static String encrypt(String xmlStr)
    {
        ...
        {
        	if(encrypt!=null)
        		md5Hasn = InfoMapEncrypt.MD5Hash(encrypt, 0, encrypt.length);
        }
        catch (Exception e)
        {
            e.printStackTrace();
        ...
        //组合消息体
        byte[] totalByte = null;
        if(null != md5Hasn && null != encrypt)
        	totalByte = InfoMapEncrypt.addMD5(md5Hasn, encrypt);

        //取密钥和偏转向量
        byte[] key = new byte[8];
        byte[] iv = new byte[8];
        getKeyIV(InfoMapEncrypt.key, key, iv);
        ...
        {
        	if(totalByte!=null)
        		temp = InfoMapEncrypt.DES_CBC_Encrypt(totalByte, deskey, ivParam);
        }
        catch (Exception e)
        {
            e.printStackTrace();
    ...
    public static String decrypt(String xmlStr) throws Exception
    {
        ...
        {
        	if(encBuf!=null)
        		temp = InfoMapEncrypt.DES_CBC_Decrypt(encBuf, deskey, ivParam);
        }
        catch (Exception e)
        {
            e.printStackTrace();
        ...
        {
        	if(temp!=null)
        		md5Hash = InfoMapEncrypt.MD5Hash(temp, 16, temp.length - 16);
        }
        catch (Exception e)
        {
            e.printStackTrace();
        ...
        return new String(temp, 16, temp==null?0:temp.length - 16, "utf-8");
    }
